...
SVD::SVD(double **a, int _m, int _n)
{
	m=_m;
	n=_n;
	u = new double*[m];
...
if (its == 999) cout<<"no convergence in 1k svdcmp iterations";
x=w[l];
nm=k-1;
y=w[nm];
g=rv1[nm];
...
double SVD::SQR(double s)
{
	return s*s;
}

double SVD::MAX(double a, double b)
{
	if(a>b)
		return a;
	else
...
double SVD::MIN(double a, double b)
{
	if(a<b)
		return a;
	else
...
double SVD::SIGN(double a, double b)
{
	int s = 1;
	if(b<0)
		s=-1;
